 ...
 */
if ( ! function_exists( 'clwp_main_nav' ) ) {
	function clwp_main_nav() {
		wp_nav_menu( array(
			'container'      => false,
			...
			'depth'          => 3,
			'fallback_cb'    => false,
			'walker'         => new CLWP_Top_Bar_Walker(),
		));
	}
}
 ...
 */
if ( ! function_exists( 'clwp_mobile_nav' ) ) {
	function clwp_mobile_nav() {
		wp_nav_menu( array(
			'container'      => false,                         // Remove nav container
			'menu'           => __( 'mobile-nav', 'clwp' ),
			'menu_class'     => 'vertical menu',
			'theme_location' => 'mobile-nav',
			'items_wrap'     => '<ul id="%1$s" class="%2$s" data-accordion-menu>%3$s</ul>',
			'fallback_cb'    => false,
			'walker'         => new CLWP_Mobile_Walker(),
		));
	}
}
...
*/
if ( ! function_exists( 'clwp_add_menuclass' ) ) {
	function clwp_add_menuclass( $ulclass ) {
		$find = array('/<a rel="button"/', '/<a title=".*?" rel="button"/');
		$replace = array('<a rel="button" class="button"', '<a rel="button" class="button"');

		return preg_replace( $find, $replace, $ulclass, 1 );
	}
	add_filter( 'wp_nav_menu','clwp_add_menuclass' );
}
...
if ( ! function_exists( 'clwp_breadcrumb' ) ) {
	function clwp_breadcrumb( $showhome = true, $separatorclass = false ) {
		...
		if ( ! is_front_page() ) {
			...
			if ( is_single() && ! is_attachment() ) {
				...
				// Paginated archives
				echo '<li class="current item-current-' . get_query_var('paged') . '">' . __('Page', 'clwp' ) . ' ' . get_query_var('paged') . '</li>';
