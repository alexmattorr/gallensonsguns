...
function clwp_pagination() {
	...
	$paginate_links = paginate_links( array(
		...
		'mid_size' => 5,
		'prev_next' => true,
		'prev_text' => __( '&laquo;', 'clwp' ),
		'next_text' => __( '&raquo;', 'clwp' ),
		'type' => 'list',
	) );
...
if ( ! function_exists( 'clwp_menu_fallback' ) ) :
function clwp_menu_fallback() {
	echo '<div class="alert-box secondary">';
	/* translators: %1$s: link to menus, %2$s: link to customize. */
	printf( __( 'Please assign a menu to the primary menu location under %1$s or %2$s the design.', 'clwp' ),
		/* translators: %s: menu url */
		sprintf(  __( '<a href="%s">Menus</a>', 'clwp' ),
			get_admin_url( get_current_blog_id(), 'nav-menus.php' )
		),
		/* translators: %s: customize url */
		sprintf(  __( '<a href="%s">Customize</a>', 'clwp' ),
...
if ( ! function_exists( 'clwp_active_nav_class' ) ) :
function clwp_active_nav_class( $classes, $item ) {
	if ( 1 === $item->current || true === $item->current_item_ancestor ) {
		$classes[] = 'active';
	}
	return $classes;
}
add_filter( 'nav_menu_css_class', 'clwp_active_nav_class', 10, 2 );
endif;
 ...
 */
if ( ! function_exists( 'clwp_active_list_pages_class' ) ) :
function clwp_active_list_pages_class( $input ) {
...
add_filter( 'wp_list_pages', 'clwp_active_list_pages_class', 10, 2 );
endif;

/**
 * Enable Foundation responsive embeds for WP video embeds
 */
if ( ! function_exists( 'clwp_responsive_video_oembed_html' ) ) :
function clwp_responsive_video_oembed_html( $html, $url, $attr, $post_id ) {
...
add_filter( 'embed_oembed_html', 'clwp_responsive_video_oembed_html', 10, 4 );
endif;
...
if ( ! function_exists( 'clwp_title_bar_responsive_toggle' ) ) :
function clwp_title_bar_responsive_toggle() {
